...
export default  class  Sline extends React.Component {
    ...
    kline(){
        ...
        $.ajax({
            type:'get',
            url:' /stock/kline',
            data:{stock_code:_this.state.code,period:this.state.type,request_num:50},
            dataType:'JSON',
            success:function(e){
                if(e.result[0].data){
                    _this.processklineData(e.result[0].data);
                }
            }
         })   
    ...
    processklineData(data){
        var temp = data ? data.splice(-40) : [],
                    realData = [];
            temp.forEach(function (value, index) {
                realData[index] = {
                    open: value.open_price - 0,
                    close: value.close_price - 0,
                    high: value.high_price - 0,
                    low: value.low_price - 0,
                    volume: value.business_amount - 0,
                    time: value.date.replace('27','2017')
                }
            });
            var _this = this;
