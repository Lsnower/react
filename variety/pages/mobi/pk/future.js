...
export default  class Pk extends React.Component{
                        ...
                        var d = m.content.data;
                             _this.setState({
                                ...
                                confirm:{
                                    ...
                                    no:'继续匹配',
                                    title:'匹配成功',
                                    content:'对战品种:'+d.varietyName+';对战时长:'+(d.endline/60)+'分钟;对战赏金:'+d.reward+(d.coinType==2?'元宝':'积分'),
                                    isOk:function(){
                                         _this.setState({
                                            confirm:{
                                                show:false
        ...
        _this.checkLogin(function(){
             var _uuid = 'match'+new Date().getTime(),
                a = {
                    code: 1200,
                    uuid:_uuid,
                    content:{cmd:'/game/battle/quickSearchForAgainst.do',parameters:{type:1}}
                };
                a = JSON.stringify(a);
                _this.websocket.send(a); 
        })
        ...
        _this.checkLogin(function(){
            var _uuid = 'joinBattle'+new Date().getTime(),
                a = {
                    code: 1200,
                    uuid:_uuid,
                    content:{cmd:'/game/battle/joinBattle.do',parameters:{battleId:id,userFrom:1}}
                };
    ...
    render(){
		...
		var a = this.state.tipasy ? 'tipmodel' : 'tipshow',
            _tx = localStorage.getItem('userPortrait')?localStorage.getItem('userPortrait')+'?x-oss-process=image/resize,m_fill,h_200,w_200':'/static/circle/headportrait64x64@3x.png';
        return(<div>
            <Header_title text='期货PK'/>
            <Head text="期货对战" />
            <div className='pk_main'>
                <InfiniteScroll next={this.getPkList} height={window.screen.height-50} hasMore={!this.state.end} loader={ <span className='more' onClick={this.toMore.bind(this)}>加载更早对战&gt;</span>} endMessage={<span onClick={this.toMore.bind(this)} className='more'>加载更早对战&gt;</span>}>

                <div className='pk_user'>
                    <img src={_tx} />
                    <span className='point'>{this.state.userInfo.credit||0}</span>
                    <span className='treasure'>{this.state.userInfo.yuanbao||0}个</span>
                ...
                <div className="pk_logo">
                    ...
                    <p><Link href='/mobi/pk/pk_history'><a className='pk_result'>查看战绩</a></Link><a className='pk_rule' onClick={this.rule}>决斗规则&gt;</a></p>
                </div>
                <div className='pk_list'>
                    {
                        this.state.list.map((e,i)=>{
                            ...
                            return <Item key={i} game={e} addPK={this.addPK} callLogin={this.checkLogin}/>
                        })
                    }
                </div>
                </InfiniteScroll>
            ...
            <div className={a}>
				<div className="tipmain">
					...
					<div className="tipcontent">
						<p>1、对战中下注金额进行冻结</p>
						<p>2、按照最后盈利情况进行结算，胜者获得对方赌注百分之九十的奖励</p>
						<p>3、平局不扣赌注，对局时间结束时用户将被清仓后进行结算</p>
						<p>4、对战无操盘记录将自动判负</p>
					</div>
				</div>    
			</div>
    ...
    render(){
        ...
        if(game.gameStatus == 1){
            return (<div className='pk_item' >
                ...
                <div className="pk_right" onClick={this.addPK.bind(this)}>
                    <img src='/static/pk/futuresvs_list_join_normal@2x.png' />
                    <p>加入对战</p>
                </div>
                <Confirm type='2' confirm={this.state.confirm} isOk={()=>{this.toAddPk(game.id,game.batchCode,game.varietyId)}} />
            ...
            return(<div className='pk_item' onClick={()=>this.toDetail(game.id)}>
                ...
                <div className="pk_left">
                    <img src={game.launchUserPortrait?game.launchUserPortrait+'?x-oss-process=image/resize,m_fill,h_200,w_200':'/static/circle/headportrait64x64@3x.png'} />
                    <p>{game.launchUserName}</p>
                    <img className='ko' style={{display:game.launchScore>=game.againstScore?'none':'block'}} src='/static/pk/futuresvs_list_ko@2x.png' />
                </div>
                <div className="pk_right">
                    <img src={game.againstUserPortrait?game.againstUserPortrait+'?x-oss-process=image/resize,m_fill,h_200,w_200':'/static/circle/headportrait64x64@3x.png'} />
                    <p>{game.againstUserName}</p>
